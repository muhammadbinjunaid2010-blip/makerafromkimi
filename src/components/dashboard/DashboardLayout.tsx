...
const menuItems = [
  ...
  { icon: Bookmark, label: "Saved Products", path: "/dashboard/saved" },
  { icon: FolderGit2, label: "My Projects", path: "/dashboard/projects" },
  { icon: FileText, label: "My Blog Posts", path: "/dashboard/blogs" },
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
        ...
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          <div className="flex flex-col items-center gap-6">
            ...
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              Access to your dashboard requires authentication.
            </p>
          </div>
          <Button
            onClick={() => {
  ...
  return (
    ...
    >
      <DashboardSidebar setSidebarWidth={setSidebarWidth} />
      <SidebarInset>
        <MobileHeader />
          ...
          <div className="flex items-center gap-3 px-2 transition-all w-full">
            ...
            {!isCollapsed && (
              <Link to="/" className="flex items-center gap-2 min-w-0">
                <span className="font-bold tracking-tight truncate text-lg">Makera</span>
              </Link>
            )}
          </div>
        </SidebarHeader>
              ...
              return (
                <SidebarMenuItem key={item.path}>
                  <SidebarMenuButton
                    ...
                    tooltip={item.label}
                    className={`h-10 transition-all font-normal ${isActive ? "bg-accent font-medium" : ""}`}
                  >
                    <item.icon className={`h-4 w-4 shrink-0 ${isActive ? "text-primary" : ""}`} />
                    <span className="flex-1">{item.label}</span>
                    {isNotif && stats && stats.unreadNotifications > 0 && (
